...
# We deprecated bootstrapACLs, and now fail in case anyone is still using it.
@test "serverACLInit/Job: fails if global.bootstrapACLs is true" {
  ...
  [ "$status" -eq 1 ]
  [[ "$output" =~ "global.bootstrapACLs is deprecated, use global.acls.manageSystemACLs instead" ]]
}
